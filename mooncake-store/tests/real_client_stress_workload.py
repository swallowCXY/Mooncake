...
#
# Mooncake v0.3.8 Python API (differs from newer branches / pr-1631 style):
# - Init: MooncakeDistributedStore.setup(...) flat args (no setup_p2p_real_client / tier JSON).
...
def build_preload_write_config(_client_mode: str):
    cfg = ReplicateConfig()
    cfg.replica_num = 1
    return cfg
...
def initialize_store(args: argparse.Namespace) -> MooncakeDistributedStore:
    ...
    if args.client_mode == "p2p":
        logger.warning(
            "client_mode=p2p: v0.3.8 Python bindings use the same setup() as centralized; "
            "ignoring P2P-only options (tier JSON, rpc port, rpc threads)."
        )
    rc = store.setup(
        args.local_hostname,
...
def stress_read_worker(
    ...
    for i in range(total_ops):
        ...
        size = store.get_into(key, ptr, args.value_size)
...
def op_sequence_worker(
    ...
    for i in range(args.warmup_ops):
        ...
        if exist_before < 0:
            continue
        if exist_before == 1:
            store.get_into(key, ptr, args.value_size)
        else:
...
def rw_correctness_worker(
    ...
    for i in range(args.test_operation_nums):
        ...
        for retry in range(max(1, args.rw_verify_retries)):
            ...
            read_buf.fill(0)
            rs = time.perf_counter()
            size = store.get_into(current_key, rptr, args.value_size)
            re = time.perf_counter()
            latency_us = (re - rs) * 1e6
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help="Client hostname:port (v0.3.8 requires port, e.g. 10.0.0.11:12345)",
    )
    p.add_argument(
        "--metadata_connection_string",
        default="http://127.0.0.1:8080/metadata",
        help="HTTP metadata URL (v0.3.8 centralized), e.g. http://master:8080/metadata",
    )
    ...
    p.add_argument(
        ...
        help="Ignored on v0.3.8 (kept for CLI compatibility with newer branches).",
    )
    p.add_argument(
        "--client_rpc_port",
        type=int,
        default=12345,
        help="Ignored on v0.3.8 (kept for CLI compatibility).",
    )
    p.add_argument(
        "--rpc_thread_num",
        type=int,
    ...
    p.add_argument(
        ...
        help="v0.3.8 uses setup() for both; p2p logs a warning and ignores P2P-only options.",
    )
