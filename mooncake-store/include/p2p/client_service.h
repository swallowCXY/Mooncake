 ...
 *
 * lease_timeout is filled by the centralized path (master grants lease); the
 * P2P path sets it to time_point::max() (P2P has no lease concept).
 */
class QueryResult {
   public:
...
class ClientService {
   public:
    ...
    /**
     * @brief stops heartbeat thread (implemented by subclass — P2P uses
     * Heartbeat thread, Centralized uses Ping thread).
     */
   ...
   public:
    ...
    virtual ViewVersionId GetViewVersion() const { return 0; }
   ...
   protected:
     ...
     * Returns the shared MasterClientInterface so base-class common methods
     * (BatchQueryIp/QueryByRegex/CalcCacheStats/Connect) can dispatch.
     * Subclasses also expose their protocol-specific master client directly.
     */
    virtual MasterClientInterface& GetMasterClient() = 0;

    /**
     * @brief Connects to the master server (non-HA direct connect).
     * HA/etcd view resolution is handled by the P2P subclass.
     */
   ...
   protected:
    // Heartbeat / keepalive — extracted to subclasses.
    // P2PClientService implements Heartbeat-based thread; CentralizedClientService
    // implements a Ping-based thread. The base class does NOT implement these.

    /**
     * @brief Starts the keepalive thread (Heartbeat for P2P, Ping for Central).
     */
    virtual void StartKeepalive(const std::string& master_server_entry) = 0;
     ...
     * P2P: master RegisterClient RPC. Central: Ping + MountSegment.
     */
    virtual tl::expected<RegisterClientResponse, ErrorCode>
    RegisterClient() = 0;

    /**
     * @brief Hook for HA-related events (P2P only; Central defaults no-op).
     */
   ...
   protected:
    ...
    // Note: heartbeat/keepalive thread + HA view members are owned by the
    // subclass (P2PClientService: Heartbeat + master_view_helper_;
    // CentralizedClientService: Ping thread). The base class does not hold
    // them.
