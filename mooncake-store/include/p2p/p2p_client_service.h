...
class P2PClientService final : public ClientService {
   ...
   protected:
    // Keepalive: Heartbeat-based (P2P master supports Heartbeat RPC)
    void StartKeepalive(const std::string& master_server_entry) override;
    void StopHeartbeat() override;

    // P2P heartbeat thread + HA state (moved from base class)
   ...
   private:
    ...
    // Heartbeat / HA state (moved from ClientService base)
