...
// ===================== RpcNameTraits (all target WrappedP2PMasterService) =====================
...
P2PMasterClient::AsyncGetReplicaList(const std::string& key,
...
P2PMasterClient::BatchGetReplicaList(const std::vector<std::string>& keys,
                                     const GetReplicaListRequestConfig& config) {
...
P2PMasterClient::CalcCacheStats() {
    return invoke_rpc<&WrappedP2PMasterService::CalcCacheStats,
...
tl::expected<void, ErrorCode> P2PMasterClient::Remove(const std::string& key) {
    ScopedVLogTimer timer(1, "P2PMasterClient::Remove");
    timer.LogRequest("key=", key);

    auto result = invoke_rpc<&WrappedP2PMasterService::Remove, void>(key);
    timer.LogResponseExpected(result);
    return result;
}
...
tl::expected<long, ErrorCode> P2PMasterClient::RemoveByRegex(
    ...
    auto result =
        invoke_rpc<&WrappedP2PMasterService::RemoveByRegex, long>(str);
    timer.LogResponseExpected(result);
    return result;
}

tl::expected<long, ErrorCode> P2PMasterClient::RemoveAll() {
    ScopedVLogTimer timer(1, "P2PMasterClient::RemoveAll");
    timer.LogRequest("action=remove_all_objects");

    auto result = invoke_rpc<&WrappedP2PMasterService::RemoveAll, long>();
    timer.LogResponseExpected(result);
    return result;
}
...
tl::expected<HeartbeatResponse, ErrorCode> P2PMasterClient::Heartbeat(
    ...
    auto result =
        invoke_rpc<&WrappedP2PMasterService::Heartbeat, HeartbeatResponse>(req);
    timer.LogResponseExpected(result);
    return result;
}
