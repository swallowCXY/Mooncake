...
WrappedP2PMasterService::BatchExistKey(const std::vector<std::string>& keys) {
...
long WrappedP2PMasterService::RemoveAll() {
...
WrappedP2PMasterService::RegisterClient(const RegisterClientRequest& req) {
    ...
    auto result = master_service_.RegisterClient(req);
...
void RegisterP2PRpcService(
    ...
    server.register_handler<&mooncake::WrappedP2PMasterService::BatchExistKey>(
        &wrapped_master_service);
    server.register_handler<&mooncake::WrappedP2PMasterService::CalcCacheStats>(
        &wrapped_master_service);
    server.register_handler<&mooncake::WrappedP2PMasterService::BatchQueryIp>(
        &wrapped_master_service);
    server
        .register_handler<&mooncake::WrappedP2PMasterService::GetReplicaListByRegex>(
            &wrapped_master_service);
    server.register_handler<&mooncake::WrappedP2PMasterService::GetReplicaList>(
        &wrapped_master_service);
    ...
    server.register_handler<
        ...
        &wrapped_master_service);
    server.register_handler<&mooncake::WrappedP2PMasterService::RegisterClient>(
        &wrapped_master_service);
