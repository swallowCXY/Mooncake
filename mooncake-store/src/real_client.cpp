...
tl::expected<void, ErrorCode> RealClient::unmap_shm_internal(
    ...
    for (auto& shm : context.mapped_shms) {
        if (shm.shm_buffer) {
            auto rc = client_service_->unregisterLocalMemory(shm.shm_buffer, true);
            if (!rc) {
...
auto result = client_service_->RegisterLocalMemory(mapped_va, mem_size, "npu",
                                                    false, true);
        if (!result.has_value()) {
            LOG(ERROR) << "Failed to register memory";
...
void RealClient::teardown_ascend_shm_buffer(MappedShm &shm) {
    ...
    (void)ContextManager::getInstance().setCurrentContextByPhysicalId(
        shm.device_id);
    if (shm.shm_size > 0 && client_service_) {
        auto res = client_service_->unregisterLocalMemory(shm.shm_buffer, true);
        if (!res) {
            LOG(WARNING)
...
tl::expected<void, ErrorCode> RealClient::unregister_shm_buffer_internal(
    ...
    if (shm_it->shm_buffer) {
        if (shm_it->is_ascend) {
            ...
            auto rc = client_service_->unregisterLocalMemory(shm_it->shm_buffer, true);
            if (!rc) {
                LOG(ERROR) << "Failed to unregister memory: "
                           << shm_it->shm_name;
                return tl::make_unexpected(ErrorCode::INTERNAL_ERROR);
